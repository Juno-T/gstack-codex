...
if [ -z "$_AUTO" ]; then
  ...
  [ -n "$_GSTACK" ] && _AUTO=$("$_GSTACK/bin/gstack-config" get auto_upgrade 2>/dev/null || true)
fi
...
[ -z "$_GSTACK" ] && [ -d "$HOME/.claude/skills/gstack" ] && _GSTACK="$HOME/.claude/skills/gstack"
[ -z "$_GSTACK" ] && [ -d "$HOME/.agents/skills/gstack" ] && _GSTACK="$HOME/.agents/skills/gstack"
[ -n "$_GSTACK" ] && "$_GSTACK/bin/gstack-config" set auto_upgrade true
```
...
[ -z "$_GSTACK" ] && [ -d "$HOME/.claude/skills/gstack" ] && _GSTACK="$HOME/.claude/skills/gstack"
[ -z "$_GSTACK" ] && [ -d "$HOME/.agents/skills/gstack" ] && _GSTACK="$HOME/.agents/skills/gstack"
[ -n "$_GSTACK" ] && "$_GSTACK/bin/gstack-config" set update_check false
```
Tell user: "Update checks disabled. Run `<active-gstack>/bin/gstack-config set update_check true` to re-enable."
...
[ -z "$LOCAL_GSTACK" ] && if [ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/gstack" ]; then
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit one of `.claude/skills/gstack` or `.agents/skills/gstack` when you're ready."
